...
class SonafemAdminSupabaseUser extends BaseAuthUser {
  SonafemAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sonafemAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SonafemAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
